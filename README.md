# コンパイルの流れ
1. Tokenize → 文字列（トークン列）を単語（トークン）に区切り、連結リストを作成
...
- `char *strndup(char *p, int len)`
  - char *p はコピー元の文字列を指すポインタで、int len はコピーする文字数
  - mallocでlen + 1 のサイズのメモリを確保。+1 は、文字列の終端を示すヌル文字（'\0'）のためのスペース。メモリ領域の先頭アドレスが buf に格納される。
  - strncpy(buf, p, len) は、文字列をコピーする関数。p から始まる文字列の最初の len 文字を buf にコピー。
  - buf[len] = '\0' で、コピーした文字列の終端にヌル文字を追加。これにより、buf は正しく終端された文字列となる。

最後に、新しく作成した文字列の先頭アドレス（buf）を返します。
...
- idiv
  ...
  - 商をRAXに、剰余をRDXに格納
- cmp
  - 第2オペランドを第1オペランドから減算し、その結果に基づいてフラグ（ゼロフラグ、符号フラグ、キャリーフラグなど）を設定
  - 結果自体は格納しない
  - ジャンプ命令と共に条件分岐に使用される
- sete
...
- lea
  - "Load Effective Address" の略
  - メモリアドレスを計算し、その結果をレジスタに格納
  - 第2オペランドにはアドレスの計算する式を入れる
  - 第1オペランドには計算結果を格納するレジスタを指定
  - メモリアクセスを伴わないので単なる算術演算として使用されることが多い
